                ...
                <div class="tab-content">
                    <div id="chartTab-1" class="tab-pane active">
                        <div class="panel-body row">
                          <div class="col-xs-12">
                            <div class="summary-title left" style="position: relative;top: 0px;">ภาพรวม: <span class="summary-title-sub">ปริมาณการปรับสภาพแวดล้อมและสิ่งอำนวยความสะดวกฯ (ปรับปรุงบ้านพักอาศัยของผู้สูงอายุ)</span></div>
                          </div>
                          ...
                          <div class="col-xs-12 col-sm-12">
                            <?php
                              $tmp_query = $this->common_model->custom_query("
                                SELECT IF(YEAR(impv_home_info.date_of_finish)+(IF(MONTH(impv_home_info.date_of_finish)>9,1,0)),YEAR(impv_home_info.date_of_finish)+(IF(MONTH(date_of_finish)>9,1,0)),'NULL') AS yyyy
                                FROM impv_home_info
                            ...
                            <table width="100%" class="summary-report-table">
                              ...
                              foreach ($tmp_query as $key => $row) {
                                ...
                                impv_home_info.consi_result
                                FROM impv_home_info
                                JOIN pers_info
                                ON impv_home_info.pers_id = pers_info.pers_id
                                WHERE
                                impv_home_info.date_of_consi IS NULL
                                AND impv_home_info.delete_user_id IS NULL
                                AND impv_home_info.consi_result IS NULL
                                AND IF(YEAR(impv_home_info.date_of_finish)+(IF(MONTH(impv_home_info.date_of_finish)>9,1,0)),YEAR(impv_home_info.date_of_finish)+(IF(MONTH(date_of_finish)>9,1,0)),'NULL') = '".($row['yyyy'])."'
                                GROUP BY impv_home_info.consi_result
                                "));
                                ...
                                impv_home_info.date_of_consi IS NULL
                                AND impv_home_info.delete_user_id IS NULL
                                AND impv_home_info.consi_result ='อนุมัติ'
                                AND IF(YEAR(impv_home_info.date_of_finish)+(IF(MONTH(impv_home_info.date_of_finish)>9,1,0)),YEAR(impv_home_info.date_of_finish)+(IF(MONTH(date_of_finish)>9,1,0)),'NULL') = '".($row['yyyy'])."'
                                GROUP BY impv_home_info.consi_result
                                "));
                                ...
                                impv_home_info.date_of_consi IS NULL
                                AND impv_home_info.delete_user_id IS NULL
                                AND impv_home_info.consi_result ='ไม่อนุมัติ'
                                AND IF(YEAR(impv_home_info.date_of_finish)+(IF(MONTH(impv_home_info.date_of_finish)>9,1,0)),YEAR(impv_home_info.date_of_finish)+(IF(MONTH(date_of_finish)>9,1,0)),'NULL') = '".($row['yyyy'])."'
                                GROUP BY impv_home_info.consi_result
                                "));
                    ...
                    <div id="chratTab-2" class="tab-pane">
                        <div class="panel-body row">
                          <div class="col-xs-12">
                            <div class="summary-title left" style="position: relative;top: 0px;">ภาพรวม: <span class="summary-title-sub">งบประมาณที่ใช้ไปในการปรับสภาพแวดล้อมและสิ่งอำนวยความสะดวกฯ (ปรับปรุงบ้านพักอาศัยของผู้สูงอายุ)</span></div>
                          </div>
                          <div class="col-xs-12 col-sm-12">
                            <!-- Styles -->
                            <style>
                          ...
                          <div class="col-xs-12 col-sm-12">
                            <table width="100%" class="summary-report-table">
                              <tr style="background-color: #4b96d6; color: white" >
                                <td rowspan="2" class="summary-report-th">ปีงบประมาณ</td>
                                <td colspan="2" class="summary-report-th">ปริมาณการให้บริการ (ราย)</td>
                                <td rowspan="2" class="summary-report-th">งบประมานที่ใช้ไป (บาท)</td>
                              </tr>
                              <tr style="background-color: #67a3d6; color: white">
                              ...
                              <?php
                              if(!empty($tmp_query) && @$tmp_query[0]['yyyy']){
                                  $total_wait = 0;
                                  $total_pay = 0;
                                  $total_amount = 0;
                                  foreach ($tmp_query as $key => $row) { ?>
                                  <?php
                                    ...
                                    $pay = rowArray($this->common_model->custom_query("
                                      SELECT
                                        ...
                                        GROUP BY impv_home_info.consi_result
                                    "));
                                    $pay_amount = rowArray($this->common_model->custom_query("
                                      SELECT
                                        COUNT(pers_info.pers_id) AS count_info,
                                        SUM(case_budget) AS amount
                                        FROM impv_home_info
                                  ...
                                  <tr>
                                    <td class="summary-report-td center"><?php echo $budget_year; ?></td>
                                    <td class="summary-report-td right"><?php echo number_format($wait['count_info']);?></td>
                                    <td class="summary-report-td right"><?php echo number_format($pay['count_info']); ?></td>
                                    <td class="summary-report-td right"><?php echo number_format($pay_amount['amount'],2); ?></td>
                                  </tr>
                                <?php
                                  $total_wait += $wait['count_info'];
                                  $total_pay += $pay['count_info'];
                                  $total_amount += $pay_amount['amount'];
                                  }
                                ...
                                <tr>
                                  <td class="summary-report-td-sum center">รวม</td>
                                  <td class="summary-report-td-sum right"><?php echo number_format($total_wait);?></td>
                                  <td class="summary-report-td-sum right"><?php echo number_format($total_pay); ?></td>
