	...
	public function assist_list_ajax($process_action='View') { // รายการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
			...
			foreach ($list as $i=>$manage_transfer) {
                ...
                $age = '';
                if($manage_transfer->date_of_birth!='') {
                ...
                if($manage_transfer->date_of_req!='' && $manage_transfer->date_of_req!='0000-00-00') { 
                    $date_of_req = '<font class="text-sucsess" color="green">'.dateChange($manage_transfer->date_of_req,5).'</font>';
                }else {
                ...
                if($manage_transfer->date_of_visit!='' && $manage_transfer->date_of_visit!='0000-00-00') {
                    $date_of_visit = '<font class="text-sucsess" color="green">'.dateChange($manage_transfer->date_of_visit,5).'</font>';
                }else{    
                    $date_of_visit = '<font class="text-sucsess" color="#B9B9B9">ยังไม่ได้ตรวจเยี่ยม</font>';
                }
                $row[] = "<center>".$date_of_visit."</center>";

                $date_of_pay = '';
                if($manage_transfer->date_of_pay!='' && $manage_transfer->date_of_pay!='0000-00-00') { 
                    $date_of_pay = '<font class="text-sucsess" color="green">'.dateChange($manage_transfer->date_of_pay,5).'</font>';
                }else{ 
                    $date_of_pay = '<font class="text-sucsess" color="#B9B9B9">ยังไม่ได้ช่วยเหลือ</font>';
                }
                $row[] = "<center>".$date_of_pay."</center>";

                $row[] = "<div style='width:100%;text-align:right;'>".number_format($manage_transfer->pay_amount,2)."</div>";

				$btn = '<a class="btn btn-default lnk" data-toggle="modal" data-target="#info'.$manage_transfer->diff_id.'"><i class="fa fa-info-circle" aria-hidden="true" style="color: #000"></i></a>';
                $btn = $btn.'<!-- Info Modal -->
                      <div class="modal fade" id="info'.$manage_transfer->diff_id.'" role="dialog">
                        ';
                        $addr = $this->personal_model->getPersonalInfo($manage_transfer->pers_id); 
                         // dieArray($addr);
                $btn = $btn.'
                        <div class="modal-dialog modal-lg" style="text-align: left; ">
                          <div class="modal-content">
                           <div class="modal-header" style="background-color: rgb(56, 145, 209); color:white;">
                              ...
                              <h3 class="modal-title text-left">'.$manage_transfer->prename_th.$manage_transfer->name.'</h3>
                            </div>
                            <div class="modal-body">
                              <div class="row">
                                ...
                                <div class="col-xs-12 col-sm-9">
                                  <div class="row">
                                    <div class="col-xs-12 col-sm-3"><h4>เลขประจำตัวประชาชน</h4></div><div class="col-xs-12 col-sm-3">'.$manage_transfer->pid.'</div>
                                  </div>
                                  <div class="row">
                                    <div class="col-xs-12 col-sm-3"><h4>วันเดือนปีเกิด</h4></div><div class="col-xs-12 col-sm-6">'.@$addr['date_of_birth'].'</div>
                                  </div>
                                  <div class="row">
                                    <div class="col-xs-12 col-sm-3"><h4>เพศ</h4> '.@$manage_transfer->gender_name.'</div>
                                    <div class="col-xs-12 col-sm-3"><h4>สัญชาติ</h4> '.@$manage_transfer->nation_name_th.'</div>
                                    <div class="col-xs-12 col-sm-3"><h4>ศาสนา</h4> '.@$manage_transfer->relg_title.'</div>
                                  </div>
                                  <div class="row">
                                  &nbsp;
                                  </div>
                ...
                $btn = $btn."<a "; 
                  if(!isset($tmp1['perm_status'])) {
                    $btn = $btn."readonly"; 
                  	...
                  	$btn = $btn.' href="'.site_url('difficult/sufferer_form1/Edit/'.$manage_transfer->diff_id).'"';
                  } 
                  $btn = $btn."";
                  if(isset($tmp['app_name'])){
                  	$btn = $btn."title='".$tmp['app_name']."'";
                  }
                  $btn = $btn.' class="btn btn-default"><i class="fa fa-pencil-square" aria-hidden="true" style="color: #000"></i>
                  </a>';
                  $btn = $btn.' <a data-toggle="modal" data-target="#prt'.$manage_transfer->pid.'" title="พิมพ์แบบฟอร์ม" class="btn btn-default">
                  ...
                  <div class="modal fade" id="prt'.$manage_transfer->pid.'" role="dialog">
                    ...
                    $btn = $btn.'>
                              		<a style="color: #333; font-size: 20px;" target="_blank" href="'.site_url('report/A1/pdf?id='.$manage_transfer->diff_id).'"><i class="fa fa-print" aria-hidden="true"></i> ';
                    if(isset($tmp1['perm_status'])) {
                    ...
                    $btn = $btn.'>';
                   	...
                   	$btn = $btn.'<a style="color: #333; font-size: 20px; margin-bottom: 50px;" target="_blank" href="'.site_url('report/A2/pdf?id='.$manage_transfer->diff_id).'"><i class="fa fa-print" aria-hidden="true"></i> ';
                   	if(isset($tmp1['perm_status'])) {
                   		$btn = $btn.$tmp1['app_name']; 
                   	}
                   	$btn = $btn.'
                    ...
                    $btn = $btn.'>';
                    $btn = $btn.'<a style="color: #333; font-size: 20px; margin-bottom: 50px;" target="_blank" href="'.site_url('report/A3/pdf?id='.$manage_transfer->diff_id).'"><i class="fa fa-print" aria-hidden="true"></i> ';
                    if(isset($tmp1['perm_status'])) {
                    	$btn = $btn.$tmp1['app_name']; 
                    }
                    $btn = $btn.'</a>
                    ...
                    $btn = $btn.'>
                              <a style="color: #333; font-size: 20px; margin-bottom: 50px;" target="_blank" href="'.site_url('report/A4/pdf?id='.$manage_transfer->diff_id).'"><i class="fa fa-print" aria-hidden="true"></i> ';
                           	if(isset($tmp1['perm_status'])) {
                           		$btn = $btn.$tmp1['app_name']; 
                           	}
                    $btn = $btn.'
                   ...
                   <!-- End Print Modal -->';
                    ...
                    if(isset($tmp['perm_status'])) {
                        if($tmp['perm_status']=='Yes') { 
                        	$btn = $btn.'<a data-id='.$manage_transfer->diff_id.' onclick="opn(this)" title="ลบ" type="button" class="btn btn-default">
	...
	public function sufferer_form1($process_action='Add',$diff_id=0) { //แบบขอรับบริการ (แจ้งเรื่อง)
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
          ...
          unset($data_update_pers['email_addr']);
					...
					// insert pers_family ///////////////////////////////////////////
					$tmp_family = @get_inpost_arr("pers_family[pers_id]");
					...
					if(!empty($tmp_family)){
						if(current($tmp_family) != ''){
							foreach ($tmp_family as $key => $rFml) {
								...
								$fml_pers_update = array(
									'marital_status' 	=> get_inpost("pers_family[marital_status][{$key}]"),
									'occupation' 			=> get_inpost("pers_family[occupation][{$key}]"),
									'mth_avg_income'	=> get_inpost("pers_family[mth_avg_income][{$key}]"),
									'edu_code' 				=> get_inpost("pers_family[edu_code][{$key}]"),
								...
								$this->common_model->update('pers_info',$fml_pers_update,array('pers_id'=>$rFml));
							}
					...
					$this->webinfo_model->LogSave($app_id,$process_action,'Sign Out','Success'); //Save Sign Out Log
					redirect('difficult/sufferer_form1/Edit/'.$id,'refresh');
          ...
          if($row['date_of_birth']!='') {
            ...
            $data['diff_info']['date_of_birth'] = formatDateThai($row['date_of_birth']).' (อายุ '.$age.' ปี)';
          	...
          	$data['diff_info']['date_of_birth'] = ' - ';
					...
					$data['diff_info']['reg_add_info'] = @"{$tmp2['addr_home_no']} หมู่ {$tmp2['addr_moo']} ต. {$tmp2['addr_sub_district']} อ. {$tmp2['addr_district']} จ. {$tmp2['addr_province']} {$tmp2['addr_zipcode']}";
            ...
            $data['diff_info']['tel_no'] = $tmp['tel_no'];  
						...
						$data['diff_info']['req_reg_add_info'] = @"{$tmp1['addr_home_no']} หมู่ {$tmp1['addr_moo']} ต. {$tmp1['addr_sub_district']} อ. {$tmp1['addr_district']} จ. {$tmp1['addr_province']} {$tmp1['addr_zipcode']}";
          ...
          unset($data_update_pers['tel_no_home']);
					...
					$this->webinfo_model->LogSave($app_id,$process_action,'Sign Out','Success'); //Save Sign Out Log
					redirect('difficult/sufferer_form1/Edit/'.$diff_id,'refresh');
				}else {
					$data['diff_info'] 			= get_inpost_arr('diff_info');
					$data['diff_info']['diff_id'] = $diff_id;
	...
	public function sufferer_form3($process_action='Add',$diff_id=0) { //แบบขอรับบริการ (สงเคราะห์)
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
          ...
          unset($data_update['date_of_receipt']);
					$data_update['date_of_pay'] 		= dateChange($data_update['date_of_pay'],4);
