	...
	class Difficult_mobile_model extends CI_Model {
            ...
            parent::__construct();
				...
				public function getUsrmUser($pid='') {
						...
						$sql = "SELECT
										...
										db_center.usrm_user.pid,
										db_center.usrm_user.passcode,
										db_center.usrm_user.user_firstname,
										db_center.usrm_user.user_lastname
										FROM
										db_center.usrm_user
										JOIN db_center.usrm_permission
										ON db_center.usrm_user.user_id = db_center.usrm_permission.user_id
										JOIN usrm_application
										ON db_center.usrm_permission.app_id = usrm_application.app_id
										WHERE
										usrm_application.app_id='81'
 										AND db_center.usrm_user.pid='".$pid."' ";
        ...
        public function getAll_diffInfo($diff_id='') {
            ...
            $res_diff = $this->common_model->custom_query($sql_diff);
				...
				#รายการผู้สูงอายุที่อยู่ในภาวะยากลำบาก
				public function getAll_diffInfoAreaCM($area_id='') {
						if($area_id != ''){
							$sql_where_diff = " AND D.addr_district='".$area_id."' ";
						}else{
							$sql_where_diff = '';
						}
						$sql_diff = "SELECT A.diff_id,B.pid,
																										...
																										LEFT OUTER JOIN std_prename C ON B.pren_code = C.pren_code
																										LEFT OUTER JOIN pers_addr D ON B.pre_addr_id = D.addr_id
																										WHERE (A.delete_user_id IS NULL && A.delete_datetime IS NULL)
																										AND (B.delete_user_id IS NULL && B.delete_datetime IS NULL)
																										AND D.addr_province='50000000'
																										".$sql_where_diff."
																										ORDER BY A.insert_datetime DESC, A.update_datetime DESC
				...
				#รายการผู้สูงอายุที่อยู่ในภาวะยากลำบาก
				public function getAll_diffInfoCM($diff_id='') {
						if($diff_id != ''){
							$sql_where_diff = " AND A.diff_id='".$diff_id."' ";
						}else{
							$sql_where_diff = '';
...
>>>>>>> 71d9a9911d6abf2844df74fb093d55aee2315f04
				public function getReportTrouble() {
							...
							foreach ($std_trouble as $row_trouble) {
												...
												$trb_title = $row_trouble['trb_title'];
												$res_diff_trouble = rowArray($this->common_model->custom_query("SELECT
																														diff_trouble.trb_code, COUNT(diff_trouble.diff_id) AS num
																														FROM diff_info
																														JOIN diff_trouble
...
<<<<<<< HEAD
				...
				public function getReport1Trouble() {
							$std_trouble = $this->common_model->custom_query("SELECT std_area.area_type, std_area.area_code, std_area.area_name_th
																															FROM std_area
																															WHERE std_area.area_type='Amphur'
																															AND std_area.area_code LIKE('50%')
																															AND std_area.area_name_th NOT LIKE('%*%')");
							foreach ($std_trouble as $row_trouble) {
												$code = $row_trouble['area_code'];
												$name = $row_trouble['area_name_th'];
												$res_diff_trouble = rowArray($this->common_model->custom_query("SELECT
																SUM(IF(diff_trouble.trb_code ='001',1,0)) AS trb_code1,
																SUM(IF(diff_trouble.trb_code ='002',1,0)) AS trb_code2,
																SUM(IF(diff_trouble.trb_code ='010',1,0)) AS trb_code10
																FROM diff_info
																JOIN db_center.pers_info
																ON diff_info.pers_id = db_center.pers_info.pers_id
																JOIN db_center.pers_addr
																ON db_center.pers_info.pre_addr_id = db_center.pers_addr.addr_id
																JOIN db_center.diff_trouble
																ON diff_info.diff_id = db_center.diff_trouble.diff_id
																WHERE pers_addr.addr_district='".$code."'
											 "));
												$arrData[] = array(	'label'=>$name,
																						'trb_code1'=>(($res_diff_trouble['trb_code1']>0)?$res_diff_trouble['trb_code1']:0),
																						'trb_code2'=>(($res_diff_trouble['trb_code2']>0)?$res_diff_trouble['trb_code2']:0),
																						'trb_code10'=>(($res_diff_trouble['trb_code10']>0)?$res_diff_trouble['trb_code10']:0)
																					);
							}
							return $arrData;
				}

				public function getReport2Trouble() {
							$std_trouble = $this->common_model->custom_query("SELECT std_area.area_type, std_area.area_code, std_area.area_name_th
																															FROM std_area
																															WHERE std_area.area_type='Amphur'
																															AND std_area.area_code LIKE('50%')
							...
							foreach ($std_trouble as $row_trouble) {
												...
												$name = $row_trouble['area_name_th'];
												$res_diff_trouble = rowArray($this->common_model->custom_query("SELECT
																SUM(IF(diff_info.date_of_visit !='0000-00-00',1,0)) as visit,
																SUM(IF(diff_info.date_of_visit ='0000-00-00',1,0)) as unvisit
																FROM diff_info
																JOIN db_center.pers_info
																ON diff_info.pers_id = db_center.pers_info.pers_id
																JOIN db_center.pers_addr
																...
																WHERE pers_addr.addr_district='".$code."'
											 "));
												$arrData[] = array('label'=>$name, 'visit'=>(($res_diff_trouble['visit']>0)?$res_diff_trouble['visit']:0),'unvisit'=>(($res_diff_trouble['unvisit']>0)?$res_diff_trouble['unvisit']:0));
							}
							return $arrData;
				}
=======
