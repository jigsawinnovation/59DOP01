...
/*					$tmp_sess = rowArray($this->common_model->custom_query("select * from authen_app_log where authen_status='Success' AND DATE(end_datetime)=DATE(NOW()) order by authen_log_id DESC"));*/

/*					$tmp_sess = rowArray($this->common_model->custom_query("select * from authen_app_log where authen_status='Success' order by authen_log_id DESC"));

					if(isset($tmp_sess['authen_log_id'])) {
						set_session('pers_authen',array('authen_log_id'=>$tmp_sess['authen_log_id'],'pid'=>$tmp_sess['pid'],'cid'=>$tmp_sess['cid'],'random_string'=>$tmp_sess['random_string'])); //for Test
					}*/
