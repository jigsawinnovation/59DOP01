...
class Member_model extends CI_Model {
    ...
    public function getedit_user($user_id=0){
        return $this->common_model->query("SELECT
                                         *
                                     FROM
                                        usrm_user
                                     LEFT JOIN std_prename ON usrm_user.user_prename = std_prename.pren_code
                                     LEFT JOIN usrm_org    ON usrm_user.org_id = usrm_org.org_id 
                                     WHERE usrm_user.user_id={$user_id}")->row_array();
