<?php
    ...
    public function getOnce_wsDemoPersonalInfo_byCode($pid=0)
    {
        $this->db_wsdemo = $this->load->database('wsdemo',true);
        $pid = str_replace("-","",$pid);
        return rowArray($this->db_wsdemo->query("select A.* from pers_info as A where A.pid='{$pid}'")->result_array());
        $this->db_wsdemo->close();
    }
    public function getOnce_wsDemoPersonalAddress_byCode($addr_code=0) {
        $this->db_wsdemo = $this->load->database('wsdemo',true);
        return rowArray($this->db_wsdemo->query("select A.* from pers_addr as A where A.addr_code='{$addr_code}'")->result_array());
        $this->db_wsdemo->close();
    }
...
/*        return rowArray($this->common_model->custom_query("select A.*,B.*,C.*,D.*,E.*,F.*,CONCAT(A.pers_firstname_th, ' ', A.pers_lastname_th) as name 
            ...
            order by A.insert_datetime DESC,
                     A.update_datetime DESC"));*/
        return rowArray($this->common_model->custom_query("select A.*,B.*,C.*,D.*,F.*,CONCAT(A.pers_firstname_th, ' ', A.pers_lastname_th) as name 
            from pers_info as A 
                                left join std_prename as B     on A.pren_code=B.pren_code 
                                left join std_gender as C      on A.gender_code=C.gender_code 
                                left join std_nationality as D on A.nation_code=D.nation_code 
